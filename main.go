...
func main() {
	...
	fmt.Println("The age of Todd is", m["Todd"])

	//another way of making a map data structure
	am := make(map[string]int)
	am["Luca"] = 14
	am["Kevin"] = 18
	//adding a key and value to a map
	am["Yusuf"] = 19
	fmt.Printf("Kevin is %vyrs old", am["Kevin"])
	...
	//for loop
	for k, v := range am {
		fmt.Printf("Key for %v\t age for %v\n", k, v)
	}
	...
	func main() {
		...
		func main() {
			...
			// bond_james := []string{`shaken, not stirred`, `martins`, `fast cars`}

			// moneypenny_jenny := []string{`intelligence`, `literature`, `computer science`}

			// no_dr := []string{`cat`, `ice cream`, `sunsets`}

			//this is the map data structure
			mhap := make(map[string][]string)
			mhap[`bond_james`] = []string{`shaken, not stirred`, `martins`, `fast cars`}
			mhap[`moneypenny_jenny`] = []string{`intelligence`, `literature`, `computer science`}
			mhap[`no_dr`] = []string{`cat`, `ice cream`, `sunsets`}
			mhap[`fleming_ian`] = []string{`steaks`, `cigars`, `espionage`}

			//this is for loop to put data from the map into the empty slice
			for k, v := range mhap {
				fmt.Println(k)
				for i, v2 := range v {
					fmt.Println(i, v2)
